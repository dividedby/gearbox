...
**Installed:** 2026-06-19, user scope, **v0.9.0** (fork) — **v1.0.0 cut on `main` 2026-06-19; reinstall pending** (`/plugin install gearbox@gearbox` + restart to pick it up) — until then CHECK 8 reports installed (0.9.0) behind latest (1.0.0). Tags: lightweight semver `v0.9.0` at `ee54fc8`, `v1.0.0` at the v1.0.0 release docs commit.  
**Doctor status:** fork **v0.9.0** ran 2026-06-19 — **all 11 checks PASS (healthy)**. CHECK 6 live dispatch logged (`Agent`/`gearbox:scout`/`haiku`, log 356→357); CHECK 8 installed == latest (0.9.0); CHECK 9 routing-prior artifact present (generated 2026-06-16); CHECK 10 status-line segment self-checks OK and now reports `WIRED`. The v0.9.0 `SubagentStop`/`UserPromptSubmit` hooks are live (R16 advisory observed firing) but are not covered by a dedicated doctor check — a future doctor-check addition could assert their registration. **v1.0.0: doctor re-run pending after reinstall** — CHECK 8 will report 0.9.0 < 1.0.0 until the install updates.  
