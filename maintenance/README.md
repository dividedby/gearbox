...
**Installed:** 2026-06-19, user scope, **v0.8.0** (fork) — latest fork `main` is now **v0.9.0**; **reinstall pending** (`/plugin install gearbox@gearbox`, restart, then `/gearbox:doctor`) to bring the installed plugin up to 0.9.0.  
**Doctor status:** last run on fork **v0.8.0** (2026-06-19) — **all 11 checks PASS (healthy)**. CHECK 6 live dispatch logged (`Agent`/`gearbox:scout`/`haiku`, log 311→312); CHECK 8 installed == latest (0.8.0); CHECK 9 routing-prior artifact present (generated 2026-06-16); CHECK 10 status-line segment self-checks OK but reports `NOT_WIRED` — the check greps `settings.json`, which references our `statusline.sh` wrapper, so it can't see the segment composed one level down (wiring is effectively present and live). Re-run doctor after the v0.9.0 reinstall and record any new warnings (the v0.9.0 `SubagentStop`/`UserPromptSubmit` hooks are not covered by an existing doctor check).  
